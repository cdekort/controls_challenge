...
def create_3d_scatter(x, y, z, costs, opacities):
    costs = [cost for sublist in costs for cost in sublist]

    populations_per_phase = [
        int(START_POPULATIONS * survival_rate) for _, survival_rate in BATCH_SIZE_STAGES
    ]
    phase_opacities = []
    for opacity, populations in zip(opacities, populations_per_phase):
        phase_opacities += [opacity] * populations
    phase_opacities += [opacities[-1]] * (len(x) - len(phase_opacities))
    min_cost, max_cost = min(costs), max(costs)
    normalized_costs = [(cost - min_cost) / (max_cost - min_cost) for cost in costs]
    color_discrete_map = [
        f"rgba({int(255 * cost)}, 0, 255, {opacity})"
        for cost, opacity in zip(normalized_costs, phase_opacities)
    ]
    ...
    py.write_image(fig, "converge.png")
    py.write_html(fig, "converge.html")
    fig.show()


opacities = np.linspace(0.1, 1, num=len(BATCH_SIZE_STAGES)).tolist()

all_pids_0 = [row[0] for row in all_pids]
all_pids_1 = [row[1] for row in all_pids]
all_pids_2 = [row[2] for row in all_pids]

create_3d_scatter(all_pids_0, all_pids_1, all_pids_2, all_costs, opacities)
